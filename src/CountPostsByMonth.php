...
class CountPostsByMonth {
	...
	public function configure_widget( $widget_id ) {
		...
		<p>
			...
			<input class="widefat" id="cs_wp_admin_post_count" name="cs_wp_admin_count_post_monthsback" type="number" value="
			<?php
			if ( isset( $cs_wp_admin_widget_options ) ) {
				echo $cs_wp_admin_widget_options;}
			?>
			" />
		</p>
