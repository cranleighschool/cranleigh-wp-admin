...
class OnlineUsersWidget {
	public function __construct() {
		...
		add_action( 'wp_dashboard_setup', array($this,'add_widget'), 109999);
		add_action('wp_network_dashboard_setup', array($this,'cs_add_network_dashboard_widgets'));

	}

	public function add_widget() {

		add_meta_box('cs_users_online_widget', 'Online Users', array($this,'cs_show_users_online_widget'), 'dashboard', 'side', 'high');
	...
	public function cs_add_network_dashboard_widgets() {
		wp_add_dashboard_widget(
			...
			array($this,'cs_show_users_online_widget')
		);
	}
	...
	public function cs_show_users_online_widget() {
		...
		if (!empty($logged_in_users)):
			...
			foreach ($logged_in_users as $key => $value):
				...
				echo "<li>".$user->display_name." (<a href=\"".get_edit_user_link($user->ID)."\">".$user->user_email."</a>) (".date('g:i:sa', $value).")</li>";
			endforeach;
			echo "</ul>";
			echo "<hr />";
